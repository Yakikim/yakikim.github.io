    ...
    eleventyExcludeFromCollections(data) {
      ...
      return data.eleventyExcludeFromCollections;
        ...
        backlinks: (data) => {
			...
			let	currentFileSlug = new String('?');
			if (data.page.filePathStem.indexOf('/posts/dafyomi/') >= 0 ) {
			currentFileSlug = data.page.filePathStem.replace('/dafyomi/', '');
			} else {
            currentFileSlug = data.page.filePathStem.replace('/posts/', '');
			}
            let backlinks = [];
			let preview ='';
